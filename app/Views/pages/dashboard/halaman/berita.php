...
<div>
    ...
    <?php endif; ?>
    <div class="p-10">
        <h1 class="font-bold text-3xl">Kelola Berita</h1>
    </div>
    <div class="container p-10 flex justify-around items-center">
        <form action="/berita" method="post" enctype="multipart/form-data" class="my-5">
            <?= csrf_field(); ?>
            ...
            <?php if (!empty($news)): ?>
                <?php foreach ($news as $index => $new): ?>
                    <tr>
                        ...
                        <td>
                            ...
                            <?php foreach ($users as $user): ?>
                                <?php if ($user['user_id'] == $new['user_id']): ?>
                    ...
                    <div class="modal" role="dialog">
                        <div class="modal-box">
                            <img src="<?= base_url('/img/news/' . $new['gambar']); ?>" alt="">
                            <h3 class="text-lg font-bold"><?= $new['judul']; ?></h3>
                            <p class="py-4"><?= $new['deskripsi']; ?></p>
                            <div class="modal-action">
                                <label for="modal_<?= $index ?>" class="btn">Close</label>
                                <form action="/berita/<?= $new['news_id']; ?>" method="get">
                                    <?= csrf_field(); ?>
                    ...
                    <dialog id="my_modal_<?= $index ?>" class="modal">
                        <div class="modal-box w-1/2  max-w-5xl">
                            <img src="<?= base_url('img/news/' . $new['gambar']); ?>" class="w-full" alt="">
                            <div class="modal-action">
                                <form method="dialog">
