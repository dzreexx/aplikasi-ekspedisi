...
<div>
    ...
    <?php endif; ?>
    <div class="p-10">
        <h1 class="font-bold text-3xl">Kelola Carousels</h1>
    </div>
    <div class="container p-10 flex justify-around items-center">
        <form action="/carousels" method="post" enctype="multipart/form-data" class="my-5">
            <?= csrf_field(); ?>
            ...
            <?php if (session()->get('errors') && isset(session()->get('errors')['deskripsi'])): ?>
                <small class="text-red-600 text-right">
                    <?= session()->get('errors')['deskripsi'] ?>
                </small>
            <?php endif; ?>
            <textarea
                placeholder="Deskripsi"
                class="textarea textarea-bordered <?php if (session()->get('errors') && isset(session()->get('errors')['deskripsi'])): ?> textarea-error <?php endif ?> textarea-lg w-full max-w-xs mt-5"
                name="deskripsi"
            ...
            <?php if (!empty($foto)): ?>
                <?php foreach ($foto as $index => $f): ?>
                            ...
                            <div class="flex items-center gap-3">
                                <div class="avatar">
                                    <button class="w-16 h-10" onclick="my_modal_<?= $index ?>.showModal()">
                                        <img
                                            src="<?= base_url('img/carousels/' . $f['nama_file']); ?>"
                    ...
                    <div class="modal" role="dialog">
                        <div class="modal-box">
                            <div>
                                <img src="<?= base_url('img/carousels/' . $f['nama_file']); ?>" alt="">
                            </div>
                            <h3 class="text-lg font-bold"><?= $f['judul']; ?></h3>
                            <!-- <p class="py-4"><?= $f['deskripsi']; ?></p> -->
                            <div class="modal-action">
                                <label for="modal_<?= $index ?>" class="btn">Close</label>
                                <form action="/carousels/<?= $f['carousels_id']; ?>" method="get">
                                    <?= csrf_field(); ?>
                    ...
                    <dialog id="my_modal_<?= $index ?>" class="modal">
                        <div class="modal-box w-1/2  max-w-5xl">
                            <img src="<?= base_url('img/carousels/' . $f['nama_file']); ?>" class="w-full" alt="">
                            <div class="modal-action">
                                <form method="dialog">
